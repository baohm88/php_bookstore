...
class BookModel
{
  ...
  public function saveBookToDB($title, $author, $description, $category_id, $price_in, $price_out, $stock_qty, $image_url)
  {
    try {
      if (isset($this->__conn)) {
        $sql = "INSERT INTO books (title, author, description, category_id, price_in, price_out, stock_qty, image_url) 
                VALUES (:title, :author, :description, :category_id, :price_in, :price_out, :stock_qty, :image_url)";
        $stmt = $this->__conn->prepare($sql);
  ...
  public function updateBookById($id, $title, $author, $description, $category_id, $price_in, $price_out, $stock_qty, $image_url)
  {
    try {
      if (isset($this->__conn)) {
        $sql = "UPDATE books
                SET title = :title, author = :author, description = :description, category_id = :category_id, price_in = :price_in, price_out = :price_out, stock_qty = :stock_qty, image_url = :image_url
                WHERE id = :id";
        $stmt = $this->__conn->prepare($sql);
        $stmt->bindParam("title", $title, PDO::PARAM_STR);
        $stmt->bindParam("author", $author, PDO::PARAM_STR);
